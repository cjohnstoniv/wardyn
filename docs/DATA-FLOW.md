...
| **Audit sinks** (`internal/audit/sinks`: `webhook.go`, `syslog.go`) | An operator-configured SIEM webhook URL or syslog endpoint (file sink is local-disk-only, no network) | The operator, opt-in per sink (off by default) | Streams the append-only audit log to the operator's own SIEM. Every event **whose Postgres write succeeded** carries its hash-chain `prev_hash`/`row_hash` (migration `0047`), so what the SIEM holds off-box is a head hash Wardyn cannot later disown — that comparison, not anything in the database, is what detects a rewritten or truncated trail (`docs/OPERATIONS.md`). The qualifier is load-bearing: the hashes are computed by the write itself, so an event spooled during a store outage streams *without* them and is not re-streamed when the spool drains — the off-box head series has a gap across an outage window, which is reconciled with `GET /audit/chain/verify`, not with the sink stream. Delivery loss on that endpoint being unreachable is a separate tracked gap, not a claim of guaranteed delivery. |
