...
| `WARDYN_SSH_PORT` | int | `2222` | host port the compose wardynd service publishes for SSH access into a running sandbox (loopback-only, mirroring `WARDYN_UP_PORT`; see docs/SSH.md — a separate lane). The mapping is always present; the feature itself stays off until that lane's gateway is wired and an operator opts in, so an unused mapped port is inert |
