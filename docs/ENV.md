...
| `WARDYN_HOST_PROXY_B64` | string (base64 JSON) | (unset) | host-side proxy detection, captured by `scripts/up.sh` running `wardyn setup detect-proxy` **on the host** and consumed by the Getting-started "Host proxy" step. A containerized wardynd is blind to every host tier (the OS/PAC tier dispatches on the *process's* GOOS, `git` is absent from the distroless image, `HOME` is unset so no shell profile or tool config is reachable), so without this the step would assert a false negative. **Diagnostics only, and deliberately not named `HTTP_PROXY`**: Go's `net/http` honors the standard names process-wide, which would silently reroute wardynd's own outbound calls (OIDC discovery, audit webhooks, GitHub App minting, the AWS credential chain) through the corporate proxy. Values are credential-masked before capture; base64 because compose interpolates `$` inside `.env` values. Unset ⇒ the step honestly reports that it could not look at the host |
...
| `WARDYN_UI_STAGE` | string | `ui-build` | which Dockerfile stage feeds the image's `/srv/ui`: `ui-build` (default — builds from source with pnpm) or `ui-prebuilt` (consumes a host-built `ui/dist`). **Normally you never set this**: when the in-image build fails, `scripts/up.sh` builds the UI on the host and retries with `ui-prebuilt` automatically. Set it explicitly only to force one stage — doing so also disables the automatic retry |
