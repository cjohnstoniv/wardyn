...
| `WARDYN_OIDC_OPERATOR_EMAILS` | CSV | (unset = **refused at boot** when OIDC is configured) | operator allowlist of the minimal viewer/operator role gate: a signed-in human whose session email is NOT listed is a **viewer** — reads everything and may launch runs, 403 on configuring the deployment (harness-credential, policy, workspace, site-config **writes**), on **secret** writes/deletes, on **deciding** an approval, and on **attaching** to a running sandbox — both lanes to a live PTY: the ticket mint (`POST /runs/{id}/attach-ticket`) and the attach WebSocket itself (`GET /runs/{id}/attach`), which falls back to session-cookie auth when no ticket is presented. `GET /secrets` (names only) and `GET /approvals` stay readable. Full addresses, case-insensitive. Admin-token and local-mode callers are always operators (one shared credential, no human to key a role off). Leaving it empty with OIDC configured makes every SSO human admin-equivalent and is refused at boot — see `WARDYN_ALLOW_OIDC_NO_OPERATOR_LIST` (flag `-oidc-operator-emails`). Also doubles as a **legacy admin source** for the separate `WARDYN_OIDC_ROLE_MAP` role gate below: an email on this list always derives role `admin` there too, so an existing deployment adopting the role map keeps its operators as admins with zero re-configuration |
