...
| _clean detach = `0`_ | `wardyn attach`: a clean detach (Ctrl-C, TERM/HUP/INT, the remote side closing) is `0`, never re-labelled as a task result — attach has no task exit code of its own to report. A FAILED attempt to open the session (rejected/unreachable) classifies with the same `2`-`5` as any other request. |
