...
| `max_holds` | `int` | `0` (→ `16`) | Only `wait_for_review`: cap on concurrent held connections (one per distinct unknown host); the next held host fails fast. `0`/absent keeps the built-in **16**. |
