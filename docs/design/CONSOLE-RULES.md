...
| Active nav item | `--sidebar-accent` fill + `--sidebar-primary` rail | `navLinkClass` (`app-shell.tsx:334–340`) and the active rail (`app-shell.tsx:379`) | A third active treatment — use the shipped one |
...
| Run / approval / health state | `--success` `--warning` `--danger` `--info` `--cyan` + their `-subtle` fills | `Chip` tones (`primitives.tsx:51–60`), shell banners (`app-shell.tsx:559`), `TruncatedNote` | Decoration. Always paired with a glyph or word — never color alone |
...
- **Persistent errors render inline** with something to read, retry, or act on:
  `ErrorState` (`states.tsx:44`) for a pane, a shell banner (`app-shell.tsx:559`),
