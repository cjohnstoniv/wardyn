...
| Active nav item | `--sidebar-accent` fill + `--sidebar-primary` rail | `navLinkClass` (`app-shell.tsx:357–363`) and the active rail (`app-shell.tsx:402`) | A third active treatment — use the shipped one |
...
| Run / approval / health state | `--success` `--warning` `--danger` `--info` `--cyan` + their `-subtle` fills | `Chip` tones (`primitives.tsx:51–60`), shell banners (`app-shell.tsx:551`), `TruncatedNote` | Decoration. Always paired with a glyph or word — never color alone |
...
- **Persistent errors render inline** with something to read, retry, or act on:
  `ErrorState` (`states.tsx:72`) for a pane, a shell banner (`app-shell.tsx:551`),
