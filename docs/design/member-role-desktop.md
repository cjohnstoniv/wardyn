...
| **Identity → role** | Local mode: no identity, caller *is* admin (`isOperator` true — `internal/api/http.go`). | The **org IdP** (an OIDC *variant profile*) authenticates the developer, and `deriveRole` (`internal/auth/oidc/derive.go`) maps them to `oidc.RoleMember`. `WARDYN_OIDC_ROLE_MAP` / the operator-emails allowlist are MDM-set; the developer is on neither, so they derive `RoleMember`. |
...
| **M5** | UI: member sees "My workspaces"; onboarding a `local_dir` shows the root constraint; admin Permissions screen unchanged (capability grants already there). Docs: OPERATIONS §Multi-user, THREAT-MODEL member-mount section. | `ui/`, `docs/OPERATIONS.md`, `threatmodel/THREAT-MODEL.md` | e2e: member onboards a dir under a root, launches, cannot reach a foreign workspace. |
