...
| Identity-affecting edit while allocated (409) | `driveRehomeGuard` | this drive is allocated, and this edit re-points the storage every allocated member is bound to — their existing data stays under the old name, and a managed drive provisions a fresh empty one. Confirm the move (confirm=rehome), or edit only the fields that do not name storage: size, mode, reclaim and storage class. |
