...
|---|---|---|---|---|
| `run.create` | Two families. **success**: a run row is created (`POST /runs`, or system-created for a follow-on workspace-step run). **failure**: the dispatch-failure family re-uses this action rather than adding one — sandbox create, a lost `sandbox_ref`, MITM CA provisioning, the subscription and Bedrock inject-grant authors, the `require_inspectable_llm` refusal, and an `api_key` grant whose scope would not compile | success: `agent`, `repo`, `policy_id`, `confinement_class`, `jti`, `inline_policy`; conditional: `interactive_start`, `seed_auto_tools`, `task_mode`, `tool_approvals`. failure: `error`, or `sandbox_ref` + `set_sandbox_ref_error` | `internal/api/runs.go:258` (create); `internal/api/runs_dispatch.go:477,519`, `internal/api/runs_dispatch_llm.go:310,450,491,528`, `internal/api/runs_create.go:616` (failure — and that last one stamps the **grant uuid** as Target, not the run id) | internal |
...
| `run.upstream_proxy.resolve` | Upstream (corporate) proxy resolution for a run | `reason` | `internal/api/runs_dispatch_mounts.go:143` | internal |
...
| `workspace.scan` | A workspace directory/repo scan (needs-scanner) runs | `detail`, `reason`, `scan_run_ids`, `sources`, `workspace_id` | `internal/api/workspace_run.go:866`, `internal/api/source_scan.go:302,343,360` | internal |
| `workspace.record` | The "workspace record" onboarding-import run completes | `anomalies`, `domains`, `kernel_sensor_blind`, `minted_grants`, `mode`, `task` | `internal/api/workspace_run.go:1043` | internal |
...
| `source.scan` | A single source's onboarding scan runs | `ai_advisor`, `ai_changed`, `confidence`, `detail`, `leak_findings`, `reason`, `scan_run_id`, `secret_reqs` | `internal/api/workspace_run.go:832`, `internal/api/source_scan.go:62` | internal |
...
| `brokered:git-pat:denied` | ANY PAT-broker refusal — an unparseable `/wardyn/git/<host>/<rest>` path, a host not in the grant's allowlist, a non-smart-HTTP verb, or a mint / host-vet / request-build failure (`internal/egress/proxy/pat_broker.go:75`, `:84`, `:101`, `:108`, `:122`, `:131`); the row names the LANE that refused, never which of the six causes | `internal/egress/proxy/pat_broker.go:48` |
| `site-config:internal-host` | An allow via the operator's `internal_hosts` lift of the private/reserved-IP guard | `internal/egress/proxy/egress_target.go:36` |
| `site-config:egress-redirect` | An allow via an EXACT literal-IP entry an `egress_redirects` `to` substitution wrote (see `docs/OPERATIONS.md`, "Egress redirects: two tiers") | `internal/egress/proxy/egress_target.go:49` |
| `brokered:llm` | A brokered Anthropic/OpenAI LLM passthrough request (`/wardyn/llm/<provider>/`) | `internal/egress/proxy/llm_routes.go:43` |
| `scan:blocked` | An LLM request refused by content inspection (`on_scanner_error=block` or a blocking finding) | `internal/egress/proxy/llm_routes.go:47` |
| `scan:opaque-tunnel` | An opaque CONNECT to an LLM host inspection could not see into — the honest coverage-gap signal, paired with a synthetic `llm.scan.blind` audit row | `internal/egress/proxy/llm_routes.go:48` |
| `scan:mitm` | An LLM request inspected via TLS-MITM interception of an otherwise-opaque CONNECT tunnel (the subscription-OAuth path) | `internal/egress/proxy/llm_routes.go:51` |
...
| `rule_source` | Meaning | Defined at |
|---|---|---|
| `builtin:upstream-proxy` | A configured corp upstream proxy is in the path; recorded once at proxy construction, the hop the SSRF re-check is deferred to | `internal/egress/proxy/proxy.go:485` |
| `builtin:private-ip` | The literal-IP guard denied a private/loopback/link-local/metadata literal no `internal_hosts` lift or `egress_redirects` `to` trusts exactly — step 0 and the post-`egressTarget` re-check alike | `internal/egress/proxy/proxy.go:572`, `:667` |
| `policy:evaluator-error` | The pluggable evaluator returned an error: fail-closed deny | `internal/egress/proxy/proxy.go:584` |
| `policy:denied` | The run policy's deny list names the host (deny beats allow) | `internal/egress/proxy/proxy.go:598` |
| `approval:denied` | A first-use approval for this host was refused | `internal/egress/proxy/proxy.go:619` |
| `approval:pending` | A first-use approval is open and undecided — the request is held, not denied | `internal/egress/proxy/proxy.go:622` |
| `policy:default-deny` | No allowlist entry names the host and no approval covers it | `internal/egress/proxy/proxy.go:630` |
| `policy:method` | The host is allowed but the request's HTTP method is not | `internal/egress/proxy/proxy.go:639` |
| `policy:allowed` | An ordinary standing allowlist allow (`allowLog`, `approvalID == Nil`) | `internal/egress/proxy/proxy.go:689` |
| `approval:<approval-id>` | Allowed by one specific approval — the UUID rides IN the value, so the trail self-joins to the approval (`allowLog`) | `approvalID.String()`, `internal/egress/proxy/proxy.go:691` |
| `builtin:dial-failed` | The vetted dial itself failed, so the earlier allow is superseded by a deny rather than over-reported (E3) | `internal/egress/proxy/proxy.go:763`, `:880` |
