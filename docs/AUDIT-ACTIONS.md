...
|---|---|---|---|---|
| `run.create` | Two families. **success**: a run row is created (`POST /runs`, or system-created for a follow-on workspace-step run). **failure**: the dispatch-failure family re-uses this action rather than adding one — sandbox create, a lost `sandbox_ref`, MITM CA provisioning, the subscription and Bedrock inject-grant authors, the `require_inspectable_llm` refusal, the declared-mechanism refusal (an `AgentProviders` row names how this agent reaches its model and the transport that resolved is not that one), and an `api_key` grant whose scope would not compile | success: `agent`, `repo`, `policy_id`, `confinement_class`, `jti`, `inline_policy`; conditional: `interactive_start`, `seed_auto_tools`, `task_mode`, `tool_approvals`, `clamp_warnings` (every way launch narrowed what the caller asked for — the run-bound record the run-detail "Effective policy" widget reads). failure: `error`, or `sandbox_ref` + `set_sandbox_ref_error` | `internal/api/runs.go:301` (create); `internal/api/runs_dispatch.go:519,561`, `internal/api/runs_dispatch_llm.go:351,491,542,597`, `internal/api/runs_dispatch_llm_mechanism.go:246`, `internal/api/runs_create.go:668` (failure — and that last one stamps the **grant uuid** as Target, not the run id) | internal |
...
| `run.fail` | Run fails before the agent starts | `error`, `from` | `internal/api/runs_lifecycle.go:467` | internal |
| `run.kill` | Operator or owner kills a run | (run state transition) | `internal/api/runs_lifecycle.go:620` | internal |
| `run.exec` | Exec into a dispatched sandbox | `argv`, `error` | `internal/api/runs_dispatch.go:671` | internal |
| `run.files` | A workspace file-browse operation on a run fails | `error` | `internal/api/run_files.go:343` | internal |
| `run.interactive` | `interactive_start` path taken | `note`, `sandbox_ref` | `internal/api/runs_dispatch.go:642` | internal |
| `run.resources` | A run-resources query/update fails | `error` | `internal/api/run_resources.go:188` | internal |
| `run.selftest` | Post-dispatch selftest gate (agent-image liveness check before a run is usable) | `confinement_class`, `detail`, `error`, `exit_code`, `fail_closed` | `internal/api/runs_dispatch.go:662` | internal |
...
| `sandbox.sweep_requested` | An OPERATOR triggered a sandbox sweep via `POST /api/v1/admin/sandboxes/sweep` — emitted on SUCCESS, recording who asked and how many were swept. Distinct from `sandbox.sweep`, which the primitive emits on failed teardown only. The boot pass runs the same primitive without this row, since nobody asked for it | `swept` | `internal/api/runs_lifecycle.go:686` | operator |
| `sandbox.sweep` | `SweepTerminalSandboxes` (the callable sweep primitive over terminal runs with a live probed sandbox) FAILS to tear one down — emitted on failed teardown only, like its boot-time sibling | `sandbox_ref`, `teardown_error` | `internal/api/runs_lifecycle.go:328` (`stopSandboxOrAudit`, `internal/api/runs_lifecycle.go:328`) | internal |
...
| `run.llm.bedrock` | Bedrock LLM credential/config wired into a run | `detail`, `hosts`, `mode`, `model`, `region` | `internal/api/runs_dispatch_llm.go:325` | internal |
| `run.llm.subscription_inject` | Subscription OAuth proxy-side inject wired into a run (see `subscription-proxy-inject` — the operator's live OAuth is injected proxy-side, never resident in the sandbox) | `detail`, `host`, `source`, `tls_mitm` | `internal/api/runs_dispatch_llm.go:499` | internal |
...
| `run.workspace.creds` | Workspace-scoped integration credential resolved for a run | `integration_ref`, `type` | `internal/api/runs.go:278` | internal |
...
| Action | When | Data fields | Where | Stable? |
|---|---|---|---|---|
| `workspace.create` | `POST /workspaces` | `name`, `owned_by`, `sources` | `internal/api/workspaces.go:485` | internal |
| `workspace.update` | `PUT /workspaces/{id}` | `image_changed`, `name`, `rescan_required`, `sources` | `internal/api/workspaces.go:632` | internal |
| `workspace.delete` | `DELETE /workspaces/{id}` | — | `internal/api/workspaces.go:946` | internal |
...
| `workspace.egress.approve` | Operator/member approves a pending workspace egress decision (`always`/`session` scope write-back), **or the boot heal replays one** — `source` tells them apart (`approval:<id>` vs `boot-heal`), and the heal also emits `failure` for a verdict the live path would now refuse | `domains`, `source` | `internal/api/approvals_writeback.go:184`, `internal/api/approvals_reconcile.go:257`, `internal/api/record.go:810` | internal |
| `workspace.egress.deny` | Operator/member denies a workspace egress decision, **or the boot heal replays one** (same `source` split as the approve row above) | `domains`, `source` | `internal/api/approvals_writeback.go:182`, `internal/api/approvals_reconcile.go:255`, `internal/api/workspaces.go:769` | internal |
| `workspace.llm_cred.set` | An operator binds (or clears) the model/harness credential for a workspace/container (`PUT /workspaces/{id}/llm-cred`) | `integration_ref` | `internal/api/workspaces.go:798` | internal |
...
| `rule_source` | Meaning | Defined at |
|---|---|---|
| `builtin:upstream-proxy` | A configured corp upstream proxy is in the path; recorded once at proxy construction, the hop the SSRF re-check is deferred to | `internal/egress/proxy/proxy.go:548` |
| `builtin:private-ip` | The literal-IP guard denied a private/loopback/link-local/metadata literal no `internal_hosts` lift or `egress_redirects` `to` trusts exactly — step 0 and the post-`egressTarget` re-check alike. An identical retry against a host already refused here is answered from a per-run memo (`privateIPMemo`, `internal/egress/proxy/private_ip_memo.go`) and counted into one summary row instead of emitting a row per attempt — see the `egress.*` row's third synthetic shape | `internal/egress/proxy/literal_ip_guard.go:78` (step 0), `internal/egress/proxy/proxy.go:749` (post-`egressTarget`) |
| `builtin:resolve-failed` | The name never resolved — a resolver outage, NXDOMAIN, or a zero-answer lookup — so no address was ever vetted. A DNS fault, NOT the address-range guard it used to be audited as: the two have opposite fixes, and this one is fixed at the sandbox's resolver rather than by declaring an `internal_hosts` lift | `internal/egress/proxy/proxy.go:736` (post-`egressTarget`, `errHostUnresolved`) |
| `policy:evaluator-error` | The pluggable evaluator returned an error: fail-closed deny | `internal/egress/proxy/proxy.go:624` |
| `policy:denied` | The run policy's deny list names the host (deny beats allow) | `internal/egress/proxy/proxy.go:637` |
| `approval:denied` | A first-use approval for this host was refused | `internal/egress/proxy/proxy.go:685` |
| `approval:pending` | A first-use approval is open and undecided — the request is held, not denied | `internal/egress/proxy/proxy.go:688` |
| `policy:default-deny` | No allowlist entry names the host and no approval covers it | `internal/egress/proxy/proxy.go:696` |
| `policy:method` | The host is allowed but the request's HTTP method is not | `internal/egress/proxy/proxy.go:661` |
| `policy:allowed` | An ordinary standing allowlist allow (`allowLog`, `approvalID == Nil`) | `internal/egress/proxy/proxy.go:772` |
...
| `approval:<approval-id>` | Allowed by one specific approval — the UUID rides IN the value, so the trail self-joins to the approval (`allowLog`) | `approvalID.String()`, `internal/egress/proxy/proxy.go:774` |
| `builtin:dial-failed` | The vetted dial itself failed, so the earlier allow is superseded by a deny rather than over-reported (E3) | `internal/egress/proxy/proxy.go:885` (CONNECT tunnel), `internal/egress/proxy/plain_lane.go:195` (plain forward), `internal/egress/proxy/llm_routes.go:305` (configured gateway did not answer), `internal/egress/proxy/llm_routes.go:416` (brokered LLM round trip) |
...
| `site_config.write` | `PUT /site-config` (full-document replace) | `agent_providers`, `egress_redirects_count`, `git_providers`, `internal_hosts_count`, `scm_hosts_count`, `sources_no_longer_admitted` (only when the body NAMED `workspace_providers` — this is the CLI/MDM door a laptop re-applies on every boot, so an MDM-tightened base URL narrows here with nobody watching a console), `storage_configured`, `upstream_proxy_configured` | `internal/api/site_config.go:613` | internal |
