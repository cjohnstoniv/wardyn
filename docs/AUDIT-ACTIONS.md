...
|---|---|---|---|---|
| `run.create` | Two families. **success**: a run row is created (`POST /runs`, or system-created for a follow-on workspace-step run). **failure**: the dispatch-failure family re-uses this action rather than adding one — sandbox create, a lost `sandbox_ref`, MITM CA provisioning, the subscription and Bedrock inject-grant authors, the `require_inspectable_llm` refusal, the declared-mechanism refusal (an `AgentProviders` row names how this agent reaches its model and the transport that resolved is not that one), and an `api_key` grant whose scope would not compile | success: `agent`, `repo`, `policy_id`, `confinement_class`, `jti`, `inline_policy`; conditional: `interactive_start`, `seed_auto_tools`, `task_mode`, `tool_approvals`, `clamp_warnings` (every way launch narrowed what the caller asked for — the run-bound record the run-detail "Effective policy" widget reads). failure: `error`, or `sandbox_ref` + `set_sandbox_ref_error` | `internal/api/runs.go:301` (create); `internal/api/runs_dispatch.go:504,546`, `internal/api/runs_dispatch_llm.go:320,460,511,566`, `internal/api/runs_dispatch_llm_mechanism.go:225`, `internal/api/runs_create.go:668` (failure — and that last one stamps the **grant uuid** as Target, not the run id) | internal |
...
| `run.llm.bedrock` | Bedrock LLM credential/config wired into a run | `detail`, `hosts`, `mode`, `model`, `region` | `internal/api/runs_dispatch_llm.go:294` | internal |
| `run.llm.subscription_inject` | Subscription OAuth proxy-side inject wired into a run (see `subscription-proxy-inject` — the operator's live OAuth is injected proxy-side, never resident in the sandbox) | `detail`, `host`, `source`, `tls_mitm` | `internal/api/runs_dispatch_llm.go:468` | internal |
