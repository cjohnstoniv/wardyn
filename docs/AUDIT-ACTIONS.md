...
|---|---|---|---|---|
| `run.create` | Two families. **success**: a run row is created (`POST /runs`, or system-created for a follow-on workspace-step run). **failure**: the dispatch-failure family re-uses this action rather than adding one — sandbox create, a lost `sandbox_ref`, MITM CA provisioning, the subscription and Bedrock inject-grant authors, the `require_inspectable_llm` refusal, and an `api_key` grant whose scope would not compile | success: `agent`, `repo`, `policy_id`, `confinement_class`, `jti`, `inline_policy`; conditional: `interactive_start`, `seed_auto_tools`, `task_mode`, `tool_approvals`. failure: `error`, or `sandbox_ref` + `set_sandbox_ref_error` | `internal/api/runs.go:258` (create); `internal/api/runs_dispatch.go:477,519`, `internal/api/runs_dispatch_llm.go:312,452,503,548`, `internal/api/runs_create.go:618` (failure — and that last one stamps the **grant uuid** as Target, not the run id) | internal |
...
| `site_config.write` | `PUT /site-config` (full-document replace) | `egress_redirects_count`, `internal_hosts_count`, `scm_hosts_count`, `upstream_proxy_configured` | `internal/api/site_config.go:454` | internal |
