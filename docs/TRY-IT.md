...
> - **`make` targets**, run instead from `~/.wardyn` (or `$WARDYN_HOME`), where
>   the installer put `docker-compose.yaml` and `.env`:
>
>   | this page | one-line install |
>   |---|---|
>   | `make setup` | the installer already did it; `docker compose up -d` re-ups |
...
>   `~/.wardyn/.env` (`# Generated by install.sh for Wardyn vX.Y.Z`), which is
>   where the installer's own upgrade check reads it from:
>
>   ```sh
>   v=v$(sed -n '1s/^# Generated by install\.sh for Wardyn v\{0,1\}\([0-9][0-9.]*[0-9]\).*/\1/p' ~/.wardyn/.env)
...
<!-- X4-F16: stale as of 0.7.4's console changes — regenerated by `make screenshots`
     (docs.spec.ts), which is the release-cut's job (RELEASING.md), not this lane's. -->
