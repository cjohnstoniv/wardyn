...
| Recording | `recording.Store` | `pg` (`fs` via `WARDYN_RECORDING_STORE=fs`) | `pg` (multi-replica HA; object storage optional at scale) | `fs`; S3/GCS object store (planned) | `recording/recordingtest` | shipped |
