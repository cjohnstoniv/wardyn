...
| Secret store | `secretstore.Store` (0.7: `For(owner)` is part of the interface — per-principal namespacing, not a `pg`-only feature); store mode adds `secretstore.External`, the backend a pointer row names | `pg` (envelope-encrypted Postgres rows) | **Vault** store mode (the organisation's Vault holds every value; OpenBao is a supported endpoint) | `vaultkv` (Vault KV v2 / OpenBao; pointer rows in Postgres, Kubernetes or token-file auth — `docs/OPERATIONS.md` "Store mode"); `azurekv` (Azure Key Vault Secrets; workload identity or managed identity, no SDK); the key that wraps a `pg` row's data key is its own seam (next row) | `secretstore/secretstoretest`, run against `pg`, `vaultkv` on a fake Vault, `azurekv` on a fake Key Vault, and live Vault OSS and OpenBao (`WARDYN_TEST_VAULT`) | shipped |
| Key-encryption key | `kek.KEK` (`internal/secretstore/kek`): wraps and unwraps each `pg` row's data key, bound to the row; the row records the provider's id as `kek_id` and a read picks the KEK by it, so rows under two keys coexist while one moves to the other | `local` (derived from `WARDYN_AGE_KEY`) | **Vault Transit** (the key never leaves Vault and every unwrap is in its audit log), or store mode above, which needs no KEK at all | `transit` (Vault Transit, OpenBao a supported endpoint; `WARDYN_KEK=transit`, over the `vaultkv` client; binds each wrap with `associated_data`, proved at boot; moved between keys and versions by the client-side `wardynd -rewrap` — `docs/OPERATIONS.md` "Key service: Vault Transit"); Azure Key Vault and AWS KMS key-wrapping (planned, 0.8.x) | `kek` golden vectors; `secretstore/secretstoretest` run through `pg` under a fake Transit; live Vault OSS and OpenBao (`WARDYN_TEST_VAULT`) | shipped (selected by `WARDYN_KEK`, not the component registry: a KEK is a part of the `pg` store, not a store) |
...
| Git provider kinds | none, and deliberately not one — `GitProviderKind` (`internal/types/workspace_provider.go`) is a **CLOSED set**: `github` and `azure_devops`, enumerated in `ClosedGitProviderKinds`, with `ClosedGitLanes` (`app`/`pat`/`ssh`) closed beside it | both kinds ship; a row's `base_urls` cover the self-hosted variants (GHES, ADO Server) of the SAME two kinds over HTTPS | same — a new forge FAMILY is a code change, not a configuration one | none, by construction | — (nothing to conform to) | no seam, on purpose. A kind is not an implementation of an interface: it names the forge-specific behaviour Wardyn already has — which credential lanes exist for it (`github` carries the App broker lane; `azure_devops` carries its own egress bundle), and which hosts it claims kind-wide (`github.com`; `dev.azure.com`/`*.visualstudio.com`). A registry here would let an operator declare a kind Wardyn has no lane, no egress bundle and no claim table for, and a security policy whose vocabulary an operator can extend is a policy whose refusals nobody can enumerate. GitLab/Bitbucket stay reachable for one release through the legacy `scm_hosts` list, and adding either as a kind is a 0.8+ code change |
...
- `internal/secretstore/{registry.go,secretstore.go}` + `secretstore/pg/register.go` + `secretstore/secretstoretest`; the KEK seam is `internal/secretstore/kek` (`KEK`, `Versioned`), with Transit in `secretstore/vaultkv/transit.go`. `Store.For(owner)` (0.7, migration `0050`) is part of the seam contract every implementation must honor — `RunConformance`'s `owner_*` cases hold a future alternate to the same fallback/isolation rules `pg` implements today.
...
- `internal/runner/runner.go` (`Capabilities.Resolved`) + `internal/api/healthz.go` (`/healthz` `confinement_substrates` + `components`).
- Planned impls (seams ready, alternates not built): the non-OCI VMM `Substrate` (SmolVM/Firecracker), SPIRE identity, OPA/Cedar evaluator, an external LLM gateway.
