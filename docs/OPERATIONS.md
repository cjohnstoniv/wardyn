...
| `GET /permissions/explain?subject_type=&subject=&kinds=` | the Explain grid (K4): for one named `user`, `group` or `user_type` subject, every kind's state — `everyone`, `this_type` (an allow, including one written for `all`), `blocked` (a deny that covers the value), `admins_only` (a widening kind that is off or has no allow), or `not_available` (an enforced narrowing kind with no allow) — at the wildcard default plus any specific value a grant names. Each cell is the resolver's own answer, switch included, for a person who is exactly that subject: only rows naming that subject or `all` are read, so a user's group rows are not included. The subject is folded the way a grant's subject is; `kinds` defaults to all seven |
