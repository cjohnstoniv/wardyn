...
# 15m, not 10m: the ephemeral-disk eviction case alone may spend opts.timeout() plus
# ephemeralEvictionBudget (7m) waiting for the kubelet, and a -timeout expiry is a panic
# that discards every verdict the other cases already produced — headroom for one slow
# eviction, not a licence for a slower suite (pinned by TestEphemeralCaseBudgetFitsTheMakefileTimeout).
test-conformance-k8s: ## Run the conformance suite on Kubernetes (needs WARDYN_TEST_K8S=1 + a kubeconfig context)
	...
	WARDYN_TEST_K8S=1 go test -v -tags k8s -timeout 15m ./test/conformance/...
...
helm-lint: ## Lint + template-render the Helm chart (default + all-on values + the refusals)
	...
	@helm template wardyn ./deploy/helm/wardyn --set auth.adminToken.secretRef.name=wardyn-auth --set secrets.ageKeyFromSecret=true --set k8s.enabled=true --set k8s.proxyImage=example/wardyn-proxy:test --set serviceAccount.automount=true --set k8s.runtimeClasses.CC2=gvisor --set serviceAccount.create=false 2>&1 | grep -q "would bind the k8s-runner privileges" || { echo "chart no longer refuses k8s.enabled with serviceAccount.create=false and no serviceAccount.name — the RBAC binding would silently fall to the namespace default ServiceAccount"; exit 1; }
	...
	@# GitOps diff sees it.
	@helm template wardyn ./deploy/helm/wardyn --set auth.adminToken.secretRef.name=wardyn-auth --set secrets.ageKeyFromSecret=true --set k8s.enabled=true --set k8s.proxyImage=example/wardyn-proxy:test --set serviceAccount.automount=true --set k8s.runtimeClasses.CC2=gvisor --set k8s.allowRunsInReleaseNamespace=true | grep -q "WARNING: k8s.runsNamespace is empty" || { echo "the k8s-runner RBAC no longer warns that an empty k8s.runsNamespace binds pods/exec + secrets delete over the whole control-plane namespace"; exit 1; }
	@helm template wardyn ./deploy/helm/wardyn --set auth.adminToken.secretRef.name=wardyn-auth --set secrets.ageKeyFromSecret=true --set k8s.enabled=true --set k8s.proxyImage=example/wardyn-proxy:test --set serviceAccount.automount=true --set k8s.runtimeClasses.CC2=gvisor --set k8s.runsNamespace=wardyn-runs | grep -q "WARNING: k8s.runsNamespace is empty" && { echo "the empty-runsNamespace warning renders even with a dedicated runs namespace set — a warning that is always on is a warning nobody reads"; exit 1; } || true
	...
	@helm template wardyn ./deploy/helm/wardyn --set auth.adminToken.secretRef.name=wardyn-auth --set secrets.ageKeyFromSecret=true --set k8s.enabled=true --set k8s.proxyImage=example/wardyn-proxy:test --set serviceAccount.automount=true --set k8s.runtimeClasses.CC2=gvisor 2>&1 | grep -q "empty k8s.runsNamespace" || { echo "chart no longer refuses k8s.enabled with an empty k8s.runsNamespace — the k8s-runner Role would silently land in the control-plane namespace, granting pods/exec and secrets delete over every workload that shares it"; exit 1; }
