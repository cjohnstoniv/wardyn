...
#
# ci-green-for-sha.sh <sha> [branch] — did CI pass on this commit?
#
...
if [[ ! "${sha}" =~ ^[0-9a-f]{40}$ ]] || [ -z "${GITHUB_REPOSITORY:-}" ]; then
  echo "usage: GITHUB_REPOSITORY=owner/name $0 <40-hex sha> [branch]" >&2
  exit 2
fi

query=(-f head_sha="${sha}" -f event=push -f per_page=100)
[ -n "${branch}" ] && query+=(-f branch="${branch}")
if ! runs="$(gh api -X GET "repos/${GITHUB_REPOSITORY}/actions/workflows/ci.yml/runs" "${query[@]}")"; then
...
if ! verdict="$(jq -r --arg sha "${sha}" --arg repo "${GITHUB_REPOSITORY}" --arg branch "${branch}" '
  [.workflow_runs[]
...
case "${verdict}" in
  ...
  none) echo "no CI push run found for ${sha}${branch:+ on ${branch}}"; exit 1 ;;
