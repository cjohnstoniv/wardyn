...
func TestPG_BootConvertsV0BeforeBootKeysAreRead(t *testing.T) {
	...
	secrets, err := buildSecretStore(ctx, pool, id.String(), nil, "", nil, 0)
	if err != nil {
	...
	if _, err := buildSecretStore(ctx, pool, id.String(), nil, "", nil, 0); err != nil {
...
func TestPG_BootAbortsOnAnUndecryptableV0Row(t *testing.T) {
	...
	_, err := buildSecretStore(t.Context(), pool, id.String(), nil, "", nil, 0)
	if err == nil {
...
func TestPG_BootRefusesAnEphemeralKeyOverAgeSealedRows(t *testing.T) {
	empty := envelopeDB(t)
	if _, err := buildSecretStore(t.Context(), empty, "", nil, "", nil, 0); err != nil {
	...
	for label, seed := range map[string]func(*testing.T, *pgxpool.Pool){
		"v1 local rows": func(t *testing.T, pool *pgxpool.Pool) {
			id, _ := age.GenerateX25519Identity()
			s, err := buildSecretStore(t.Context(), pool, id.String(), nil, "", nil, 0)
			if err != nil {
				t.Fatal(err)
			}
		...
		t.Run(label, func(t *testing.T) {
			pool := envelopeDB(t)
			seed(t, pool)
			_, err := buildSecretStore(t.Context(), pool, "", nil, "", nil, 0)
