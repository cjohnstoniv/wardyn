...
func TestBuildSecretStore_StoreModeBootsWithNoAgeKey(t *testing.T) {
	...
	s, err := buildSecretStore(t.Context(), pool, "", nil, vaultkv.Name, storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
	...
	s2, err := buildSecretStore(t.Context(), pool, "", nil, vaultkv.Name, storeClients{ext: ext}, rec)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMigrateMode_AuditsAnAbort(t *testing.T) {
	...
	s, err := newSecretStore(t.Context(), pool, id.String(), nil, "", storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReconcileMode_Audits(t *testing.T) {
	pool := envelopeDB(t)
	ext := &memExternal{vals: map[string][]byte{}}
	s, err := newSecretStore(t.Context(), pool, "", nil, vaultkv.Name, storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
		t.Fatal(err)
	}
	if err := s.Put(t.Context(), "k", []byte("v")); err != nil {
