...
func TestBuildSecretStore_StoreModeBootsWithNoAgeKey(t *testing.T) {
	...
	s, err := buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: ext})
	if err != nil {
	...
	s2, err := buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: ext})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildSecretStore_StoreModeRefusesWhileLocalRowsRemain(t *testing.T) {
	...
	_, err = buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: &memExternal{vals: map[string][]byte{}}})
