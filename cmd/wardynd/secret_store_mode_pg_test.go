...
func TestBuildSecretStore_StoreModeBootsWithNoAgeKey(t *testing.T) {
	...
	s, err := buildSecretStore(t.Context(), pool, "", nil, vaultkv.Name, ext, 0)
	if err != nil {
	...
	s2, err := buildSecretStore(t.Context(), pool, "", nil, vaultkv.Name, ext, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildSecretStore_StoreModeRefusesWhileLocalRowsRemain(t *testing.T) {
	...
	local, err := buildSecretStore(t.Context(), pool, id.String(), nil, "", nil, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = buildSecretStore(t.Context(), pool, "", nil, vaultkv.Name, &memExternal{vals: map[string][]byte{}}, 0)
