...
func TestBuildSecretStore_StoreModeBootsWithNoAgeKey(t *testing.T) {
	...
	s, err := buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
	...
	s2, err := buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: ext}, rec)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildSecretStore_StoreModeRefusesWhileLocalRowsRemain(t *testing.T) {
	...
	_, err = buildSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: &memExternal{vals: map[string][]byte{}}}, &capturingRecorder{})
...
func TestMigrateMode_AuditsAnAbort(t *testing.T) {
	...
	s, err := newSecretStore(t.Context(), pool, id.String(), "", storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReconcileMode_Audits(t *testing.T) {
	pool := envelopeDB(t)
	ext := &memExternal{vals: map[string][]byte{}}
	s, err := newSecretStore(t.Context(), pool, "", vaultkv.Name, storeClients{ext: ext}, &capturingRecorder{})
	if err != nil {
		t.Fatal(err)
	}
	if err := s.Put(t.Context(), "k", []byte("v")); err != nil {
