...
func warnUpstreamProxyNoBypass(ctx context.Context, st upstreamProxyNoBypassStore, llmGateways map[string]string) {
	...
	for publicHost, base := range llmGateways {
		...
		slog.Warn("wardynd: BOOT-TIME READ — an upstream proxy is configured but no upstream_proxy_no_proxy entry covers this internal model gateway host, so at the moment this process started every brokered call to it will be CONNECTed through the upstream proxy instead of dialled directly; this can silently time out on an estate whose upstream cannot reach the gateway, and re-checks only on the next restart since SiteConfig is admin-editable without one",
			slog.String("public_host", publicHost), slog.String("gateway_host", gwHost))
	}
}
