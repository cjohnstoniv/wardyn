...
func openSecretStore(ctx context.Context, pool *pgxpool.Pool, f *bootFlags, rec audit.Recorder) (secretstore.Store, error) {
	...
	return buildSecretStore(ctx, pool, *f.ageKey, platform, *f.secretStoreSel, ext, *f.vault.timeout, rec)
}
...
func buildSecretStore(ctx context.Context, pool *pgxpool.Pool, ageKey string, platform *age.X25519Identity, storeName string, ext secretstore.External, extTimeout time.Duration, rec audit.Recorder) (secretstore.Store, error) {
	s, err := newSecretStore(ctx, pool, ageKey, platform, storeName, ext, extTimeout, rec)
	if err != nil {
		return nil, err
	}
...
// by name. platform is the separate platform identity (readPlatformKey), or
// nil. ext is the configured external client, or nil, and extTimeout bounds
// each call to it. The store is returned unwrapped: a serving boot reads
// through buildSecretStore, and only the maintenance modes use it bare, for
// the pg store's Migrate and Reconcile, which never Get
// (secretStoreMaintenance).
func newSecretStore(ctx context.Context, pool *pgxpool.Pool, ageKey string, platform *age.X25519Identity, storeName string, ext secretstore.External, extTimeout time.Duration, rec audit.Recorder) (secretstore.Store, error) {
