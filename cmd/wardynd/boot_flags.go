...
func parseBootFlags() *bootFlags {
	...
	f := &bootFlags{
		...
		anthropicBaseURL:        flagEnv("anthropic-base-url", "WARDYN_ANTHROPIC_BASE_URL", "", "internal model gateway base URL (https://) re-pointing Anthropic's brokered upstream instead of api.anthropic.com, for both the api-key lane and subscription runs; the harness-login lane is exempt. Empty (default) uses the public host"),
		...
		oidcOperatorEmails: flagEnv("oidc-operator-emails", "WARDYN_OIDC_OPERATOR_EMAILS", "", "comma-separated operator (admin) emails; a signed-in human not listed is a member. Empty with OIDC configured is refused at boot unless -allow-oidc-no-operator-list is set"),
		...
		allowTestEndpoints:     flagBool("allow-test-endpoints", "WARDYN_ALLOW_TEST_ENDPOINTS", false, "acknowledge this is a TEST deployment; unlocks -aws-sso-endpoint-override and a plain http:// -bedrock-base-url, both refused otherwise. Never set on a deployment holding a real credential (default false)"),
