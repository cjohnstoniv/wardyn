...
// still denied at the literal-IP guard" as an unqualified fact. The guard
// gates on net.ParseIP, which accepts only IPv4 dotted-decimal, IPv6, and
// IPv4-mapped IPv6 — every POSIX inet_aton spelling of the same address
// (127.1, 0x7f000001, 2130706433, 010.0.0.1) returns nil and proceeds as an
// ordinary hostname. On the corp-upstream lane egressTarget hands that string
// to the corp proxy verbatim, whose own resolver can parse it with inet_aton
// semantics and land on the blocked address anyway — so the bound as
// originally stated promised a denial the guard cannot deliver for those
// spellings.
func TestThreatModelDocLiteralIPBoundNamesNonCanonicalResidual(t *testing.T) {
	// (1) The premise: step 0 gates on net.ParseIP, nothing wider.
	eval := methodBody(t, readSrc(t, "internal", "egress", "proxy", "proxy.go"), "evaluate")
	if !strings.Contains(eval, "net.ParseIP(") {
		t.Fatalf("evaluate no longer gates the literal-IP guard on net.ParseIP — re-derive the doc's residual before trusting this guard")
	}
	if strings.Contains(eval, "inet_aton") || strings.Contains(eval, "ParseUint") {
		t.Fatal("evaluate now normalizes non-canonical IP spellings before the guard — the residual this test pins may be closed; re-check the doc before trusting this skip")
	}

	// (2) The corp-upstream branch still forwards the raw host verbatim, which
	// is what lets a non-canonical spelling reach an external resolver.
	target := methodBody(t, readSrc(t, "internal", "egress", "proxy", "egress_target.go"), "egressTarget")
	if !strings.Contains(target, "net.JoinHostPort(host, strconv.Itoa(port)), \"\", nil") {
		t.Fatalf("egressTarget's upstream branch no longer forwards the raw host — re-derive the doc's residual before trusting this guard")
	}

	// (3) The doc must state the qualifier and the residual, and must not carry
	// the old unqualified promise on its own.
	...
	mustSay(t, doc, "threatmodel/THREAT-MODEL.md",
		...
		"127.1", "0x7f000001", "2130706433", "010.0.0.1",
		"inet_aton",
	)
}
