...
// redirectAttachIO swaps os.Stdin/os.Stdout/os.Stderr for pipes the test
// controls, mirroring the os.Stdin swap TestRunAttach_CtxCancelRestoresTerminal
// already relies on (see its own comment on why: go test's real os.Stdin is
// not a blocking source, so Half 2 would race a premature EOF-driven cancel
// in ahead of the frames these tests need Half 1 to process first). The
// stdin write end is intentionally never closed by the returned cleanup —
// only by the caller once the exchange it is gating is over — for the same
// reason.
