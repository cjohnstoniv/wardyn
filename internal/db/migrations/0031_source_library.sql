...
-- (dropping workspaces.sources/base_image and the scan-owned columns) ships in
-- a LATER release, never this one. (It was once penciled in as "0032", but the
-- migration-contiguity rule means numbers are claimed in landing order —
-- 0032/0033 went to ssh_public_keys and attach_ticket_role — so the drop takes
