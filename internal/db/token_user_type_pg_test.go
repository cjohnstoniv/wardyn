...
// TestPG_TokenUserTypeBackfillsStandard applies 0082 over tokens minted before
...
func TestPG_TokenUserTypeBackfillsStandard(t *testing.T) {
	pool, schema := partialSchemaPool(t, "0082")
	...
	if err := Migrate(ctx, pool); err != nil {
		t.Fatalf("Migrate applying 0082+ over 0.7 tokens: %v", err)
	}
