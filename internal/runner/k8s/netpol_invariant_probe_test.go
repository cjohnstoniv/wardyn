...
// hypothesis H1 of the F9 trace: a value dispatch places in spec.Env (which
// internal/api/runs_dispatch.go's resolveEnvSecretGrants does with REAL stored
// secrets for env_secret grants) lands as an inline, API-readable Value on
// the agent pod spec. This test is expected to FAIL on the current tree — a
// RED result confirms H1; GREEN means the driver now routes spec.Env through
// the Secret (secretKeyRef) or another non-inline carrier.
func TestProbe_F9_H1_AgentEnvSecretsAreAPIReadable_EXPECTED_RED(t *testing.T) {
	...
	_, cs, _, sb := probeCreate(t, func(s *runner.SandboxSpec) {
		s.Env["MY_TOKEN"] = secretVal // shape of resolveEnvSecretGrants' sandboxEnv[name] = string(val)
	})
	agentPod := probeGetPod(t, cs, sb.Ref)
