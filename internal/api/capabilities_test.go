...
func TestCapabilityKindsAreTheClosedSet(t *testing.T) {
	want := []string{"egress_host", "secret", "workspace", "image", "agent", "integration", "workspace_provider", "model_provider", "feature"}
...
func TestCapValueMatchesIsExactOffTheHostLane(t *testing.T) {
	for _, kind := range []string{capSecret, capWorkspace, capImage, capAgent, capIntegration, capWorkspaceProvider, capModelProvider, capFeature} {
