...
func TestCapabilityKindsAreTheClosedSet(t *testing.T) {
	want := []string{"egress_host", "secret", "workspace", "image", "agent", "integration", "workspace_provider", "policy"}
...
func TestCapValueMatchesIsExactOffTheHostLane(t *testing.T) {
	for _, kind := range []string{capSecret, capWorkspace, capImage, capAgent, capIntegration, capWorkspaceProvider, capPolicy} {
