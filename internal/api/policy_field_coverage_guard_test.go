...
var runPolicySpecCoverage = map[string]policyFieldCoverage{
	...
	"PushRules": {validated: true, clamped: true,
		why: "#176: deny_paths per-entry byte/charset shape (no count cap — see maxPushRulesPathBytes' own doc for why, same stance as denied_domains), max_inspect_pack_mib range-checked. It only NARROWS what a push may touch, so an operator-silent ceiling leaves a proposal's own push_rules untouched; a ceiling that sets one (pushRulesIsSet — an all-zero push_rules:{} does not count) is a FLOOR — an unset proposal inherits it wholesale, a set one gets the ceiling's deny_paths unioned in EXACT-STRING (unionPaths, not union — a git path is case-sensitive) and max_inspect_pack_mib capped. composer.Grade adds a medium-risk item when it is set (same pushRulesIsSet guard) and the run's only git grant is ssh_key (unenforceable, not unsafe). The matcher that actually reads deny_paths, and the rest of the enforcement, are a later change"},
}
