...
// unchanged record resolves, and its answer expires soon enough that the proxy
// asks again within about a minute.
func TestProviderKeySink_ReadsTheRecordByUID(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if exp.Before(before.Add(providerKeyRecheck)) || exp.After(time.Now().Add(providerKeyRecheck)) {
				t.Errorf("expires_at = %v, want now+%v so the proxy re-checks the record", exp, providerKeyRecheck)
			}
		})
	}
}
