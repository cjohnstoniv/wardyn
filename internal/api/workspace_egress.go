...
func substituteArtifactEgress(domains []string, sc types.SiteConfig) []string {
	...
	for _, r := range sc.EgressRedirects {
		...
		// function, so a To that names no port trusts exactly the 443 the MITM
		// half already assumed for it — a mirror reached on some other port needs
		// that port in the To, which is the same thing the token injection has
		// always required.
