...
func TestSecurityAdminRouteTier(t *testing.T) {
	...
	// revoke (= 32 SEC), and #612 the GET/PUT /permissions/availability pair
	// beside the grant rows (= 34 SEC). A route silently reclassified in the
	...
	if sec != 34 || super != 39 {
		t.Errorf("tier split = %d security / %d admin, want 34 / 39 (§B's 14 SEC + governance's 7 + §I's directory search + the device inventory and revoke + the enrolment-token list and revoke + the 4 /user-types routes + the 2 /permissions/availability routes, MINUS record, PLUS #168's 3 moved /drives routes; and 26 SUPER + /drives' 7 + record + the four operator-topology reads + 0.7.2's GET/PUT /workspace-providers and GET/PUT /agent-providers + the device enrolment-token mint, MINUS the reclassified POST /setup/harness-login, MINUS #168's 3 moved /drives routes)", sec, super)
	}
}
