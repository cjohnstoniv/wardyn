...
func TestSecurityAdminRouteTier(t *testing.T) {
	...
	// the security tier (= 26 SEC). CS-5 added the credential erase, which
	// only subtracts, on the security tier (= 27 SEC). 0.8's model providers
	// add GET/PUT /model-providers, SUPER for the agent roster's reason
	// (= 41). A route silently reclassified in the table above would still
	// pass every probe — it would just be enforcing the WRONG tier, exactly
	// the drift the per-route loop cannot see.
	if sec != 27 || super != 41 {
		t.Errorf("tier split = %d security / %d admin, want 27 / 41 (§B's 14 SEC + governance's 7 + §I's directory search + the device inventory and revoke + the credential erase, MINUS record, PLUS #168's 3 moved /drives routes; and 26 SUPER + /drives' 7 + record + the four operator-topology reads + 0.7.2's GET/PUT /workspace-providers and GET/PUT /agent-providers + the device enrolment-token mint + 0.8's GET/PUT /model-providers, MINUS the reclassified POST /setup/harness-login, MINUS #168's 3 moved /drives routes)", sec, super)
	}
}
