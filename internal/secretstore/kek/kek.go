...
// that id. Only `local` exists today; Vault/OpenBao Transit and Azure Key Vault
// are the next two, each a Wrap/Unwrap over its own service's API.
package kek
...
// aad is AAD_kek = Encode("wardyn/kek/v1", owner, name, kek_id). A bind
