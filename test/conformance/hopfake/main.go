...
// Command hopfake stands in for wardynd's proxy-facing TLS listener in the
// docker conformance case TestProxyHopTLSDocker: it serves TLS with the leaf
// the test minted from its own internal CA (internal/hoptls), answers one
// injection grant for one run token, and logs every request with whether it
// arrived over TLS. A TEST BINARY: the test builds it and copies it into a
// throwaway busybox container; nothing packages or publishes it.
package main
