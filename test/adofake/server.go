...
// with the token as the password) carry exactly scopes — replacing any
...
// checkScope reports whether the request's token carries scope. An empty
// scope means the endpoint needs none, and is always granted.
func (s *Server) checkScope(r *http.Request, scope string) (token string, granted bool) {
	...
	s.mu.Lock()
	defer s.mu.Unlock()
	return token, s.tokens[token][scope]
}
...
// so a caller can tell "wrong scope" from any other 401.
...
func (s *Server) handler() http.Handler {
	...
	// Pull requests, refs, pushes — org-scoped by repository id, as the real
	// git API allows.
	mux.HandleFunc("POST /{org}/_apis/git/repositories/{repoId}/pullrequests",
		s.requireScope(EndpointPullRequestsPost, ScopeCodeWrite, s.handlePullRequestsPost))
	mux.HandleFunc("PATCH /{org}/_apis/git/repositories/{repoId}/pullrequests/{prId}",
		s.requireScope(EndpointPullRequestsPatch, ScopeCodeWrite, s.handlePullRequestsPatch))
	mux.HandleFunc("POST /{org}/_apis/git/repositories/{repoId}/refs",
		s.requireScope(EndpointRefsPost, ScopeCodeWrite, s.handleRefsPost))
	mux.HandleFunc("GET /{org}/_apis/git/repositories/{repoId}/pushes",
		s.requireScope(EndpointPushesGet, ScopeCodeRead, s.handlePushesGet))
	mux.HandleFunc("POST /{org}/_apis/git/repositories/{repoId}/pushes",
		s.requireScope(EndpointPushesPost, ScopeCodeWrite, s.handlePushesPost))

	// Work item tracking — project-scoped.
