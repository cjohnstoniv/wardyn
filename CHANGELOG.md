...
- **"Available to" on the console: the git provider row, the agent roster row, an org workspace
  and a stored policy (#619).** Each editor gets one Everyone/Only control, backed by
  ...
  refused, and the server's own sentence renders verbatim. Not yet wired: the integration row and the
  base image row (neither has a dedicated admin editor in the console today) and the model provider
  row (its own editor, MP-19a, hasn't shipped).
