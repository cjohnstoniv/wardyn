...
- A member refused because their group snapshot is incomplete is told what to do about it. `GET /secrets` answered with the bare internal condition name and nothing else, so `wardyn secret list` printed a word the member has no way to act on; it now carries the same "sign in again (or re-mint your API token)" remedy every other refusal of that kind already gave.
