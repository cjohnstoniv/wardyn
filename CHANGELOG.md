...
- An operator can trust a corporate TLS-inspecting proxy's root CA (`WARDYN_TRUSTED_CA_FILE`): the daemon, the proxy sidecar and every sandbox pick it up, delivered on compose, the desktop profile and the Helm chart.
- The admin's Getting Started gains a **People** step naming single-user vs multi-user access; a member signing into someone else's Wardyn gets their own Getting Started page — what's already set up for them, adding a workspace, bringing their own model key, their first run, approvals they can decide, and connecting their tools.
- Audit rows show which rule decided them — allowed by policy, released by an approval, refused by a built-in guard, brokered, or a declared internal host — beside the row itself, not just through the raw audit API.
...
### Fixed
- The published base image's exec-mode runs (including the setup connectivity probe) failed TLS with "no internet" on a network requiring the corporate CA, because the image never installed the trust bundle the CA knob delivers — it does now.
- A member could briefly land on the wrong first screen because the app routed before it knew their real role, treating them as an admin until `/me` resolved.
### Changed
