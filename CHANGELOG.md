...
- **`upstream_proxy_url` is validated by the sidecar's own rule, not a narrower copy.** A value the control plane accepted but `wardyn-proxy` refuses at startup (a port outside 1..65535, a missing host) saved clean at `PUT /api/v1/site-config`, audited `run.upstream_proxy.resolve` as a success, and then exited the sidecar of every run in the deployment — leaving the agent with no egress path and the cause only in a dead sidecar's log. The write gate and the dispatch gate now both call `proxy.ValidUpstreamProxyURL`. (R3 F028)
...
- **An egress redirect trusts its target on the port it named, not every port of the address.** The allowlist entry a site-config redirect adds is now port-qualified (443 when the `to` names none) — matching the port its TLS termination and token injection already used — so a `to` on a literal IP no longer opens 22 or 5432 on that address. (R3 F106)
