...
- **The injection sink trusts the model provider record, never the grant (#531).** Each resolve of
  ...
  after six minutes, so the proxy re-checks about once a minute while the run makes model calls,
  and a provider removed, turned off or re-pointed mid-run fails closed on the run's next model
  call rather than keep its startup copy. Once a provider block is set, or when it cannot be
  read, the two legacy shared subscription sentinels are refused before the operator's token is
  touched; the boot gateway (`WARDYN_ANTHROPIC_BASE_URL`) no longer widens where a subscription
  token may go under a block.
