...
- **The literal-IP guard covers the spellings a resolver accepts and Go does not.** `127.1`, `0x7f000001`, `2130706433` and `0251.0376.0.1` are all `127.0.0.1` to `inet_aton`, and with a corporate upstream configured they were handed to the operator's proxy unvetted; both the guard in `evaluate` and `egressTarget`'s upstream branch now re-run the block check on the `inet_aton` reading. Deny only — a spelling the operator did not type inherits no `allowed_domains` grant. (R3 F105)
