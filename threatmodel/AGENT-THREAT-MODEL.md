...
| 8 | **Lateral movement** | one run reaches another, or the host | 🟡 **partial.** Per-run networks with no default route; on Kubernetes a default-deny NetworkPolicy verified by a boot-time canary that refuses to start if it does not hold. Bypass class: two operator overrides boot the canary past a failure it would otherwise refuse to start on — `WARDYN_K8S_ALLOW_UNENFORCED_NETPOL` (the CNI does not enforce NetworkPolicy at all) and `WARDYN_K8S_ACK_AMBIENT_DEFAULT_DENY` (an ambient default-deny in the runs namespace blocks the canary's own reachability check) — both disclosed in THREAT-MODEL.md §5 |
