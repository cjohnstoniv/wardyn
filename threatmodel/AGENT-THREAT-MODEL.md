...
**Portable on purpose.** This document is about the class of system — a
model-driven agent executing tools on someone's behalf — not about Wardyn. The
categories, terms and ownership lines are meant to be usable by a team running
something else entirely.

Every row still carries a **Wardyn coverage verdict**, because a taxonomy with no
verdict is a whitepaper. The taxonomy travels; the verdict column is the honest
local answer.
...
implementation: its assets, trust boundaries, and 27 dated residual risks. That
...
Pinned because each of these already means two or three things in ordinary usage,
and a shared model cannot survive that.
...
**Four axes, not one ladder.** These are routinely collapsed and they measure
different things:
...
A control that is strong on one axis says nothing about the others. Most
overclaiming in this space is an axis substitution.
...
| 9b | **Supply chain — content executing inside a run** | a malicious package's install script runs | **contained, not prevented.** Nothing scans it; it collapses into category 1 and is answered by containment |
...
| 14 | **Resource abuse / denial of wallet** | the agent burns money rather than data | **partial.** CPU, memory, PID and disk caps are enforced and fail closed if the runtime discards them; run lifetime is bounded. **There is no token-spend or model-call budget anywhere** — an agent holding a valid model credential can exhaust it |
...
control. Four owners, and the assignments that surprise people are the point.
...
**Reporting:** a category with no verdict, or a verdict the code does not
support, is a bug under [SECURITY.md](../SECURITY.md)'s overclaim clause — not a
documentation nit.

**A note on the shape of this document.** It carries more `not addressed` and
`partial` verdicts than `mitigated` ones. That asymmetry is deliberate and is the
reason to trust the `mitigated` rows.
