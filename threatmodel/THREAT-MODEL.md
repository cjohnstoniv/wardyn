...
and it grew by two in v0.7, one in v0.7.2 and one in v0.8. Seven NARROW what a
...
server-side clones, Scan and Build) and — v0.8 — `policy` (which stored policy
a member may select, `req.PolicyID`; the choice only, since the selected row is
still clamped to their ceiling). `workspace`, `agent`, `integration` and `policy`
are enforced at `denyMemberRequest`, on launch and preflight alike. `workspace_provider` is deliberately a bound on
