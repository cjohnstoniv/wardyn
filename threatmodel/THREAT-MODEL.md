...
4. **Source code + the git push capability** — the minted GitHub installation
   ...
   and denies them, so the brokered route is the only route to those four host
   NAMES — previously a sandbox could dial `github.com:443` directly, an opaque
   CONNECT the pkt-line parser cannot read — and `wardyn-git-helper` no longer
   mints an installation token into a brokered sandbox at all. It is a NAME deny:
   the verdict keys on the host string the sandbox asked for, so `ssh.<forge>` is
   a different name (below) and a raw-IP `CONNECT` is a different key, which
   `allow_all_egress` would permit. See `docs/POLICIES.md`.
   ...
   those are bounded by the operator who supplied the credential. The SSH key is
   written for the clone only — `wipe_ssh_grants` shreds it and unsets
   `GIT_SSH_COMMAND` before the agent starts — but that is a narrowing, not a
   confinement: the grant id rides `WARDYN_SSH_GRANTS`, an auto-mintable grant is
   re-mintable by design, and the proxy's mint route refuses only *brokered
   GitHub* grant ids, so the agent can re-mint the key for itself. Because the
   four denies are exact hosts, `ssh.<forge>:443` also stays allowlisted on that
   run. A token exfiltrated from the proxy itself is likewise unconstrained.
   Token-side confinement needs GitHub branch-protection rulesets
   **[v0.5+ — planned]**.
...
| Over-broad or replayed minted credentials | Down-scoped at mint (repo + permission, audience-bound per RFC 8707, 1h TTL) **[shipped]**; kill-switch cascade on run end **[shipped]**. Bot-branch-only push confinement is **[shipped, default-on]** on the brokered git path (push-ref inspection in `internal/egress/proxy/git_broker.go`; `agent-run` names the run branch `wardyn/<run-id>/work` so a stock run complies, `WARDYN_GIT_BROKER_ENFORCE_BRANCH_NS=false` opts out), and dispatch makes the brokered route the only route to four GitHub host **names** — it subtracts + denies `github.com`, `api.github.com`, `codeload.github.com`, `*.githubusercontent.com` for a run with git grants, and the credential helper refuses to mint an installation token into a brokered sandbox. **The parser binds the brokered App lane and nothing else, and one lane sits beside it on the same run.** `git_pat` (opaque CONNECT) and `ssh_key` (not smart-HTTP) carry no refs the proxy can read, so no receive-pack parser can bind them; and because those four denies are EXACT hosts, an `ssh_key` grant for `github.com` leaves `ssh.github.com:443` allowlisted on the very same brokered run (`sshOver443Endpoint`) — that run has a push path to the same forge that the parser never sees, and the resident key is re-mintable from inside the sandbox for as long as the run lives (the mint route refuses only brokered *GitHub* grant ids). Those lanes are bounded by the operator who supplied the credential, not by Wardyn — §5.1a states each bound, and `docs/POLICIES.md` states which lane a given policy puts a run in. A token-side/GitHub-ruleset property that would hold even for a leaked token is **[v0.5+ — planned]** — the token itself is still repo-scoped but NOT branch-scoped. | B4, B5, ID |
...
|---|---|---|---|
| `ssh_key` grant (SSH SCM lane) | The stored SSH **private key**, as a file the `ssh` client reads | git's SSH transport has no credential-helper seam (`credential.helper` is HTTP-only) | Written `0400` agent-owned at clone time and shredded right after the clone (`wipe_ssh_grants`, before the agent process starts); mask-registered at mint. **That window is a narrowing, not a bound:** the grant id rides `WARDYN_SSH_GRANTS` in the sandbox env, an auto-mintable grant is re-mintable by design (`MintForGrant`), and the proxy's mint route refuses only brokered *GitHub* grant ids (`isBrokeredGitGrant`) — so the agent process can re-mint the same key at any point in the run, and `ssh.<forge>:443` is allowlisted for the whole run. Wardyn cannot down-scope or expire an SSH private key (`internal/broker/broker.go` `mintSSHKey`, `deploy/images/*/agent-run`). |
