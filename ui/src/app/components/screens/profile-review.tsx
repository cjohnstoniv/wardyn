    ...
    <div className="space-y-5 p-5">
            ...
            (inline_policy.eligible_grants?.length ?? 0) === 0 ? (
              ...
              <div className="flex flex-wrap gap-1">
                {inline_policy.eligible_grants!.map((g, i) => (
                  <Chip key={i} tone="info" mono className="px-1.5 py-0 text-meta">
                    {String(g.kind)}
                  </Chip>
                ))}
              </div>
      ...
      {warnings && warnings.length > 0 && (
        <div className="rounded-lg border border-warning/30 bg-warning-subtle p-3">
          <div className="mb-1 text-meta font-medium uppercase tracking-wide text-warning">
            Warnings
          </div>
    ...
    <div className="space-y-3" aria-label="Observations">
      ...
      <div className="rounded-lg border border-border">
        <div className="flex items-center gap-2 border-b border-border px-3 py-2 text-xs text-muted-foreground">
          ...
          <span className="rounded-full bg-muted px-1.5 text-meta">{domains.length}</span>
        </div>
        {domains.length === 0 ? (
              ...
              <li key={d.host} className="px-3 py-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    {d.allow_count > 0 && (
                      <Chip tone="success" className="px-1.5 py-0 text-meta">
                        {d.allow_count} allow
                      </Chip>
                    )}
                    {d.deny_count > 0 && (
                      <Chip tone="danger" className="px-1.5 py-0 text-meta">
                        {d.deny_count} deny
                      </Chip>
                    )}
                    {d.pending_count > 0 && (
                      <Chip tone="warning" className="px-1.5 py-0 text-meta">
                ...
                {d.methods && d.methods.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {d.methods.map((m) => (
                      <span
                        key={m}
                        className="rounded border border-border bg-surface-2 px-1 font-mono text-meta text-muted-foreground"
                      >
                        {m}
                      </span>
                    ))}
      ...
      >
        <div className="flex items-center gap-2 text-danger">
          ...
          <span className="rounded-full bg-danger/15 px-1.5 text-meta text-danger">
            {anomalies.length}
          </span>
        </div>
        {anomalies.length === 0 ? (
  ...
  return (
    <div className="rounded-lg border border-border">
      <div className="flex items-center gap-2 border-b border-border px-3 py-2 text-xs text-muted-foreground">
        ...
        <span className="rounded-full bg-muted px-1.5 text-meta">{items.length}</span>
      </div>
      {items.length === 0 ? (
        ...
        <ul className="space-y-1 px-3 py-2">
          {items.map((it, i) => (
            <li key={`${it}-${i}`} className="break-all font-mono text-xs text-foreground">
              {it}
            </li>
          ))}
        </ul>
    ...
    <li className="flex items-start gap-3 px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex flex-wrap items-baseline gap-x-2">
          ...
          <span className="font-mono text-meta text-muted-foreground">= {item.value}</span>
          {item.invariant_ref && (
        ...
        <p className="mt-0.5 text-meta leading-snug text-muted-foreground">{item.rationale}</p>
      </div>
    </li>
  );
}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-1.5 text-meta uppercase tracking-wide text-muted-foreground">{children}</div>
  );
}
