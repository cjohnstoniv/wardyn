...
describe("ComposeQandA", () => {
  ...
  // N3: the question rail lets the operator jump straight to the last
  // question — answering it satisfies Continue's OWN gate (currentAnswered,
  // the CURRENT question only) even though an earlier question is still
  // blank, and submit() used to bail with a bare `return`. It must instead
  // jump back to the first unanswered question and say why.
  it("jumps to the first unanswered question and explains, instead of silently no-opping", async () => {
    const q1: ComposeQuestion = { id: "q1", question: "Q1?", why: "", options: [], multi: false };
    const q2: ComposeQuestion = { id: "q2", question: "Q2?", why: "", options: [], multi: false };
    const q3: ComposeQuestion = { id: "q3", question: "Q3?", why: "", options: [], multi: false };
    const onSubmit = vi.fn();
    render(
      <ComposeQandA
        questions={[q1, q2, q3]}
    ...
    // Answer Q1, jump straight to Q3 via the rail (skipping Q2), answer it,
    // then click Continue — Continue's own gate only looks at Q3.
    await user.type(screen.getByLabelText("Q1?"), "answer one");
    await user.click(screen.getByText("3. Q3?"));
    await user.type(screen.getByLabelText("Q3?"), "answer three");
    await user.click(screen.getByRole("button", { name: /continue/i }));

    expect(onSubmit).not.toHaveBeenCalled();
    // Landed back on the first unanswered question (Q2) with an inline note.
    expect(screen.getByLabelText("Q2?")).toBeInTheDocument();
    expect(screen.getByTestId("qanda-unanswered-note")).toHaveTextContent("Question 2 is unanswered.");

    // Answering it clears the note. We're back on Q2 (not the last question),
    // so the primary button reads "Next" — advance to Q3 (still holding its
    // earlier answer) where it reads "Continue" again, and submit for real.
    await user.type(screen.getByLabelText("Q2?"), "answer two");
    expect(screen.queryByTestId("qanda-unanswered-note")).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /next/i }));
    await user.click(screen.getByRole("button", { name: /continue/i }));
    expect(onSubmit).toHaveBeenCalledWith([
      { question: "Q1?", answer: "answer one" },
      { question: "Q2?", answer: "answer two" },
      { question: "Q3?", answer: "answer three" },
    ]);
  });
