...
describe("StepBuild — Carries (agent-CLI bake honesty)", () => {
  it("shows the Carries chip once built, when the wizard says this build bakes an agent tool", async () => {
    buildWorkspaceMock.mockResolvedValue({ state: "done", image: "wardyn-workspace/ws-1:abc123" });
    render(<StepBuild workspaceId="ws-1" onStateChange={vi.fn()} agentTools={["claude-code"]} />);
  ...
  it("renders no Carries row when nothing was named (the default — no prop passed)", async () => {
    buildWorkspaceMock.mockResolvedValue({ state: "done", image: "wardyn-workspace/ws-1:abc123" });
    render(<StepBuild workspaceId="ws-1" onStateChange={vi.fn()} />);
  ...
  // resolveBuildView sets `detail` on a "done" response ONLY for the
  // repo-own-devcontainer caveat — that lane builds the repo's devcontainer
  // as-is and bakes nothing, so it must win over the client's own agentTools
  // preview, and its text should explain the gap rather than leaving it
  // silent.
  it("a server-side repo-own-devcontainer caveat (build.detail) suppresses the chip even if agentTools says otherwise", async () => {
    buildWorkspaceMock.mockResolvedValue({
      state: "done",
      image: "wardyn-workspace/ws-1:devcontainer",
      detail: "this repo carries its own devcontainer, built as-is — Wardyn does not add claude-code to it",
    });
    render(<StepBuild workspaceId="ws-1" onStateChange={vi.fn()} agentTools={["claude-code"]} />);
    ...
    expect(screen.queryByText("Carries")).not.toBeInTheDocument();
    expect(
      screen.getByText("this repo carries its own devcontainer, built as-is — Wardyn does not add claude-code to it"),
    ).toBeInTheDocument();
  });
});
