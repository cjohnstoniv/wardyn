...
function ReasonDialog({
  ...
  // parent's setPrompt(null)); false when it failed so we re-enable the button.
  ...
  // HIGH fix (error handling): always reset busy in finally so a failed
  // approve/deny doesn't leave the button spinning forever — the user can retry
  // or cancel after the toast reports the error.
  const submit = async () => {
    setBusy(true);
    try {
    ...
    <Dialog open={!!prompt} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {/* MEDIUM fix: approving authorizes the mint — the broker mints the
                short-lived scoped token later, in a separate transaction. Don't
                imply the token is minted at the moment of approval. */}
        ...
        <DialogFooter>
          ...
          >
            ...
            {/* MEDIUM fix: approval does NOT mint — it AUTHORIZES the broker to
                mint later in a separate transaction. Don't over-claim "& mint". */}
