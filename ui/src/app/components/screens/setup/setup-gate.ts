...
// every route redirects to the funnel. Unlike the old funnel-completion gate
// removed in 0.5, which demanded the tour be finished, this one asks only that
// the install can work at all: the daemon decides which rows mean that
// (0.7.8), a grade alone never gates, and demos and optional steps stay
...
// gated routes, and a gate that re-fires on every navigation would bounce its
// own funnel back to step one. Module-level rather
...
export function setupGateActive(
  ...
  // "a place you go, not a wall you are trapped behind". Its failing checks
  // stay visible on every surface; what they stop doing is confiscating the
  // console. The gate's job is the first run: a fresh install does not open
  // on an unexplained, unusable board when the daemon itself says something
...
// The daemon decides which check ids are install defects (0.7.8) — there is
// no id list here to keep in sync. An id list would enumerate a family
// instead of naming the property, and drift every time a new provider check
// (llm_provider, bedrock_provider, harness_credential_aws, …) got added.
...
// Visited-step set — steps the operator has navigated away from at least
...
// Neither of the two flags above gates anything (the hard gate is
...
// own import) and GET /healthz carry no such value today (every field on
// each is a deliberate disclosure decision, and inventing one is a backend
// change outside this lane). Traded a rare false-green (a
