...
// onboarding tour → SetupScreen, the full 12-step Next walk in STEP_ORDER
// (essentials [environment, corp_network, integrations] → demos [4 sub-steps]
// → your work [sources, images, workspaces] → finish [review, launch]), honest
// not-ready gating on a runner-less host, and the Finish-later dismissal.
//
...
// images their own steps ("Your work" is now sources → images → workspaces,
// not just workspaces) — see steps.ts's PHASES comment for the same history.
//
...
test.describe("Getting Started funnel", () => {
  test("walks all twelve steps via Next in STEP_ORDER", async ({ page }) => {
    await openSetupFunnel(page);
    const main = page.getByRole("main");
    const nextBtn = page.getByRole("button", { name: /^Next:/i });

    // STEP_ORDER (steps.ts PHASES): essentials [environment, corp_network,
    // integrations] → demos(4) → your work [sources, images, workspaces] →
    // finish [review, launch]. Corporate network (host proxy + egress
    // redirection) sits between the barrier and Integrations; Integrations
    // itself keeps only the model/harness + SCM-host picker.
    // 1 environment
    ...
    // 8-10 your work: directories & repos, base images, workspaces
    await nextBtn.click();
  ...
  test("launch stays gated on a runner-less host (no fake green)", async ({ page }) => {
    ...
    const nextBtn = page.getByRole("button", { name: /^Next:/i });
    // 11 Next clicks total: 1 to reach corp_network, then 10 more once its
    // gate is cleared, to walk the remaining 10 steps to Launch — see
    // passCorpNetworkGate above.
    await nextBtn.click();
    await passCorpNetworkGate(page);
    for (let i = 0; i < 10; i++) await nextBtn.click();
    await expect(main.getByRole("heading", { name: /launch your first run/i })).toBeVisible();
  ...
  test("'Finish setup' at the flow's end completes to Runs; no early exit", async ({ page }) => {
    ...
    // Walk to the final (Launch) step and complete via "Finish setup". 11
    // Next clicks total, same as above — see passCorpNetworkGate above.
    const nextBtn = page.getByRole("button", { name: /^Next:/i });
    await nextBtn.click();
    await passCorpNetworkGate(page);
    for (let i = 0; i < 10; i++) await nextBtn.click();
