 ...
 * including the operator, can ever read back. The masked ENTRY stays here as
 * workspace prep (beat 4). PROVING that claim from inside a sandbox — the
 * `write-only-by-design` demo — MOVED to episode 03, which now films the whole
 * demos surface (both groups, every card). No agent run is launched; running
